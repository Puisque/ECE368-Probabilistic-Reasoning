...
def discrimAnalysis(x, y):
    ...
    cov_female = [[cov_female_0_0/num_female, cov_female_0_1/num_female], [cov_female_0_1/num_female, cov_female_1_1/num_female]]
...
def misRate(mu_male,mu_female,cov,cov_male,cov_female,x,y):
    ...
    for i in range(0, x.shape[0]):
      male_qda.append(- 1/2*np.log(np.linalg.det(cov_male)) - 1/2*np.dot(x[i], np.dot(np.linalg.inv(cov_male), x[i].T)) + np.dot(mu_male.T, np.dot(np.linalg.inv(cov_male), x[i].T)) - 1/2*np.dot(mu_male.T, np.dot(np.linalg.inv(cov_male), mu_male)))
      female_qda.append( - 1/2*np.log(np.linalg.det(cov_female)) - 1/2*np.dot(x[i], np.dot(np.linalg.inv(cov_female), x[i].T)) + np.dot(mu_female.T, np.dot(np.linalg.inv(cov_female), x[i].T)) - 1/2*np.dot(mu_female.T, np.dot(np.linalg.inv(cov_female), mu_female))) 
